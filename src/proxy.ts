// ════════════════════════════════════════════════
// src/proxy.ts  ──  Next.js Proxy（原 middleware）
//
// 每個請求進來時都會先經過這裡。
...
//
// 【安全架構】
// - proxy：驗證 token 是否真實有效（第一層，擋假 token）
// - 各 API route 的 requireAuth / requireAdmin：做細部權限檢查（第二層）
// - Supabase RLS policy：資料庫層保護（第三層）
//
// 【注意】
// proxy 跑在 Edge Runtime，可以用 fetch 但不能用 Node.js API。
// ════════════════════════════════════════════════
...
export async function proxy(req: NextRequest) {
  ...
  // proxy 跑在 server 端讀不到 localStorage，
